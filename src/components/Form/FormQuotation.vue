      ...
      <v-row justify="center">
        <v-col lg="6" md="6" sm="12" cols="12">
            ...
            <v-row class="no-gutters">
              ...
              >
                ...
                <template v-slot:item="{ item }">
                  <tr>
                    <td>{{item.name}}<div>update {{item.update}}</div></td>
                    <td>{{item.food_des}}</td>
                    <td><v-btn small elevation="1" @click="selectedPackage(item)">เลือก</v-btn></td>
                  </tr>
                </template>
              </v-data-table>
            </v-row>
          ...
          <v-row>
            <v-col lg="12" md="12" sm="12" cols="12">
              <v-row class="no-gutters">
                <div class="sizehead">แพ็กเกจที่ท่านเลือก<span class="text-sm-body-2 red--text"> (เลือกได้แค่แพ็กเกจเดียว)</span>
                </div>
              </v-row>
            </v-col>
          </v-row>
              ...
              >
                <template v-slot:item="{ item }">
                  <tr>
                    <td>{{item.name}}<div>update {{item.update}}</div></td>
                    <td>{{item.food_des}}</td>
                    <td><v-btn small elevation="1" @click="DeletePackage(item)">ลบ</v-btn></td>
                  </tr>
                </template>
          ...
          <v-row>
            <v-col lg="12" md="12" sm="12" cols="12">
              <div class="sizehead">
                เครื่องดืม
                <span class="text-sm-body-2 grey--text">เลือก {{check_select_drink}} เหลือ {{package_amount_drink}}</span>
              </div>
              <v-row class="no-gutters">
                <v-combobox
              ...
              <v-row>
                <v-col lg="6" md="6" sm="12" cols="12">
                  <v-row class="no-gutters">
                    ...
                    <span class="text-caption grey--text mt-2">{{promotion_table_all_total}} รายการ</span>
                  </v-row>
                </v-col>
                ...
                <v-col lg="6" md="6" sm="12" cols="12">
                  <v-row class="no-gutters">
                    ...
                    <span class="text-caption grey--text mt-2">{{promotion_table_select_total}} รายการ</span>
                  </v-row>
                </v-col>
              </v-row>
                    ...
                    <v-data-table
                      ...
                      <template v-slot:item="{ item }">
                        <tr>
                          <td>{{ item.name }}</td>
                          <td>
                            <v-btn small elevation="1" @click="selectedPromotion(item)">เลือก</v-btn>
                          </td>
                        </tr>
                      </template>
                    </v-data-table>
                    ...
                    <v-data-table
                      ...
                      <template v-slot:item="{ item }">
                        <tr>
                          <td>{{ item.name }}</td>
                          <td>
                            <v-btn small elevation="1" @click="deletePromotion(item)">ลบ</v-btn>
                          </td>
                        </tr>
                      </template>
                      </v-data-table>
...
export default {
  ...
  data: () => ({
    ...
    select_sweet_food: [],
    select_drink:[],
    savory_food_items:['ขนมจีนน้ำเงี้ยว','ยำวุ้นเส้น','หมูไข่พะโล้','ลาบหมู','แกงอ่อมไก่','แกงเขียวหวานไก่','แกงจืดหน่อไม้ไก่','ยำถั่วพู','ต้มข่าไก่','ต้มยำกุ้ง'],
    sweet_food_items: ['กล้วยเชื่อม','วุ้นกะทิ','บัวลอยไข่หวาน','ฟักทองแกงบวด','สังขยา','สาคูเปียกมะพร้าวอ่อน','ถั่วกวน','ขนมโค','ขนมหยกมณี','ถั่วเขียวต้มน้ำตาล'],
    drink_items: ['กาแฟ','น้ำทับทิม','น้ำแตงโม','โอวัลติน','น้ําโค้ก','ชาเย็น','นํ้าส้ม','ชาเขียว','นํ้าใบบัวบก','น้ำอัญชันมะนาว'],
      ...
      searchListCustomer (val) {
       ...
       select_savory_food (val) {
        ...
        if (val.length >  this.package_amount_savory_food) {
          ...
          alert("'ไม่สามารถเลือกอาหารคาวได้อีก' เนื่องจากครบจำนวนตามแพ็กเก็จแล้ว");
        }
      },
       select_sweet_food (val) {
        ...
        if (val.length > this.package_amount_sweet_food) {
          ...
          alert("'ไม่สามารถเลือกอาหารหวานได้อีก' เนื่องจากครบจำนวนตามแพ็กเก็จแล้ว");
        }
      },
       select_drink (val) {
        ...
        if (val.length > this.package_amount_drink) {
          ...
          alert("'ไม่สามารถเลือกเครื่องดื่มได้อีก' เนื่องจากครบจำนวนตามแพ็กเก็จแล้ว");
        }
      },
    },
    ...
    async loadDataCustomer(){
        ...
        if(this.CreateorEdittype ==false){
            ...
            this.Quotation_GatProvince.push({province_Name: result.data.result.customers_data[0].province ,province_Code: result.data.result.customers_data[0].province_code});
            this.Quotation_SelectAmphoe={amphoe_Name: result.data.result.customers_data[0].amphoe ,amphoe_Code:result.data.result.customers_data[0].amphoe_code};
            this.Quotation_GatAmphoe.push({amphoe_Name: result.data.result.customers_data[0].amphoe ,amphoe_Code: result.data.result.customers_data[0].amphoe_code});
            this.Quotation_SelectDistrict= {district_Name: result.data.result.customers_data[0].district ,amphoe_Code: result.data.result.customers_data[0].district_code};
            this.Quotation_GatDistrict.push({district_Name: result.data.result.customers_data[0].district ,amphoe_Code: result.data.result.customers_data[0].district_code});
            ...
            this.Package_ID=result.data.result.packages_data[0].id
    ...
    async FindCustomer(selectListCustomer){
        ...
        let result = await api.getCreateCustomerQuotation(DataFindCustomer);
            this.Quotation_type_id = result.data.result[0].id;
            ...
            this.Quotation_SelectProvinces= {province_Name: result.data.result[0].province ,province_Code: result.data.result[0].province_code};
            this.Quotation_GatProvince.push({province_Name: result.data.result[0].province ,province_Code: result.data.result[0].province_code});
            this.Quotation_SelectAmphoe={amphoe_Name: result.data.result[0].amphoe ,amphoe_Code:result.data.result[0].amphoe_code};
            this.Quotation_GatAmphoe.push({amphoe_Name: result.data.result[0].amphoe ,amphoe_Code: result.data.result[0].amphoe_code});
            this.Quotation_SelectDistrict= {district_Name: result.data.result[0].district ,amphoe_Code: result.data.result[0].district_code};
            this.Quotation_GatDistrict.push({district_Name: result.data.result[0].district ,amphoe_Code: result.data.result[0].district_code});
  ...
  async loadDataPackage(){
        ...
        selectedPackage(item){
          if(this.package_selected_items.length <= 0){
            ...
            alert('เลือกแพ็กเกจได้แพ็กเกจเดียว');
          }
        },
        DeletePackage(item){
  ...
  async loadDataPromotion(){
        ...
        selectedPromotion(item){
          if(this.promotion_table_selected_items.length <= 0){
            ...
            this.promotion_selectd_id.push(item.id)
            this.promotion_table_all_total = this.promotion_table_items.length
            this.promotion_table_select_total = this.promotion_table_selected_items.length
          }else{
            alert('เลือกโปรโมชั่นได้โปรโมชั่นเดียว');
          }
        },
