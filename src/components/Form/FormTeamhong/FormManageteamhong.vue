    ...
    <v-row>
      <v-col>
        <v-card flat color="#E5E5E5">
          <div class="header-title">{{ Changesubmit }}สร้างรายชื่อทีม Hong</div>
          <br />
      ...
      <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <v-row justify="center" style="display: flex; grid-gap: 500px;">
            <v-col xl="2" lg="2" md="3" sm="12" cols="12">
                <v-btn block large rounded outlined @click="$router.push({ name: 'menuManageteamhong' })">ยกเลิก</v-btn>
            </v-col>
            <v-col xl="2" lg="2" md="3" sm="12" cols="12">
  ...
  methods: {
    async loadTeams() {
      ...
      if (this.CreateorEdit == true) {
        let result = await api.getListtoCreateTeamhong();
        this.teams_data_staffs = result.data.result.teams;
        this.teams_table_all_total = this.teams_data_staffs.length;
      } else {
        let res = await api.getListtoCreateTeamhong();
        this.teams_data_staffs = res.data.result.teams;
        this.teams_table_all_total = this.teams_data_staffs.length;
        let EditTeamCode = {
          team_code: this.$store.getters["BNP_DATA"].databnp
        };
        let result = await api.getListTeamhongtoEdit(EditTeamCode);
        // console.log(result);
        if (!result.data.result.length) {
          window.location.href = `${process.env.VUE_APP_SUB_PATH}/teamhong/Manageteamhong`;
    ...
    async submit() {
      if (this.$refs.form.validate() == true) {
        ...
        if (this.CreateorEdittype == true) {
          ...
          let resultCreateNewTeam = await api.addNewTeamhong(CreateNewTeam);
          ...
          let resultEditstaff = await api.editTeamhong(Editstaff);
