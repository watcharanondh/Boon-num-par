                      ...
                      <v-row class="no-gutters">
                        ...
                        >
                          <template v-slot:item="{ item }">
                              <tr>
                                <td>{{ item.name }}</td>
                                <td class="info--text" >{{ item.stock_balance }}</td>
                                <td>
                                  <v-btn small elevation="1" color="success" @click="selectedAddEquipment(item)" >เพิ่ม</v-btn>
                                </td>
                              </tr>
                            </template>
                        </v-data-table>
                    ...
                    >
                      <template v-slot:item="{ item }">
                          <tr>
                            ...
                            <td>
                              <v-btn small elevation="1" color="error" @click="DeleteEquipment(item)"  >ลบ</v-btn>
                            </td>
                          </tr>
                        </template>
                        </v-data-table>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      if (this.CreateorEdit == true) {
        ...
        this.equipment_selected_items= Data_Seleted_Edit_Equipment.map( val =>{
              return {equipment_code:val.equipment_code ,name:val.name ,stock_balance:val.stock_balance ,amount:val.amount }
        })
    ...
    selectedAddEquipment(item){
        var items = this.equipment_selected_items.find(val => val.equipment_code == item.equipment_code);
      if (items){
          ...
          this.equipment_selected_items.push({equipment_code:item.equipment_code ,name:item.name ,stock_balance:item.stock_balance, amount:1})
      }
