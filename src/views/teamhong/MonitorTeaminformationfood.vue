    ...
    <v-col>
      <v-row  justify="end">
        ...
        <v-btn
          class="mx-10"
          color="#C4C4C4"
          @click="$router.push({ name: 'menuShipping' })"
          rounded
        >
    ...
    <v-card class="mx-10 pa-5 rounded-lg" outlined>
        <v-row justify="center">
          <v-col lg="9" md="9" sm="12" cols="12">
            ...
            <!-- ทีมจัดไลน์อาหาร -->
              <v-col lg="4" md="4" sm="12" cols="12">
                <div>ทีมจัดไลน์อาหาร</div>
                <v-row class="no-gutters">
                  <v-text-field
                    v-model="lineup_food_team"
                    disabled
                  ></v-text-field>
                </v-row>
              </v-col>
...
export default {
  name: "MonitorTeaminformationfood",
  async mounted() {
  ...
  methods: {
    async londDataTeaminformation() {
      ...
      let result = await api.listshippingtosee(sentQuotation_code);
      // console.log('ดู',result);
      if (!result.data.result.length) {
          window.location.href = `${process.env.VUE_APP_SUB_PATH}/teamhong/Shipping`;
      ...
      this.Place_arrangement=result.data.result[0].event_date;
      this.lineup_food_team=result.data.result[0].lineup_food_team;
    },
  },
};
</script>
